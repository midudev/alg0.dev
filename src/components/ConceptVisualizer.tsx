...
function BigOChart({ state }: { state: BigOState }) {
  ...
  return (
    <div className="flex-1 flex flex-col items-center justify-center gap-5 w-full min-h-0">
      ...
      >
        {/* Background */}
        <rect x={PAD.left} y={PAD.top} width={chartW} height={chartH} fill="var(--subtle)" rx="4" />
      ...
      <div className="flex flex-wrap justify-center gap-x-5 gap-y-2">
        {curves.map((curve) => (
          ...
          >
            ...
            />
            <span style={{ color: curve.visible ? curve.color : 'var(--viz-muted)' }}>
              {curve.name}
            </span>
          </div>
        ))}
            ...
            return (
              <div key={i} className="flex items-center shrink-0">
                ...
                <svg
                  ...
                  viewBox="0 0 28 12"
                  className="shrink-0"
                  style={{ color: isLast ? 'var(--viz-muted)' : colors.text }}
                >
